...
async function ensureDbReadyOnce(env) {
  ...
  dbInitPromise = (async () => {
    ...
    // Turso 环境：使用 Turso 数据库（libSQL/SQLite 兼容）
    if (platform === "turso" || env?.TURSO_DATABASE_URL) {
      logger.info("检测到 Turso 环境，初始化 Turso 连接");

      // 动态导入 Turso 适配器（仅在需要时加载）
      const { createTursoAdapterFromEnv } = await import(`${"."}/src/adapters/TursoAdapter.js`);

      try {
        dbAdapter = await createTursoAdapterFromEnv(env);
        await ensureDatabaseReady({ db: dbAdapter, env, providerName: "turso" });
        logger.info("Turso 数据库连接成功，环境已就绪");
        return dbAdapter;
      } catch (error) {
        logger.error("Turso 连接失败", error);
        throw new Error(`Turso 连接失败: ${error.message}。请确保已正确配置 TURSO_DATABASE_URL, TURSO_AUTH_TOKEN 环境变量。`);
      }
    }
