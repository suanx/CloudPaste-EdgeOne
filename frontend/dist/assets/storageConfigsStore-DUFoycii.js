import { av as defineStore, g as ref, F as computed, E as api, c as createLogger } from "./index-BQxzU9F1.js";
const log = createLogger("StorageConfigsStore");
const CACHE_TTL = 5 * 60 * 1e3;
const FETCH_LIMIT = 200;
const useStorageConfigsStore = defineStore("storageConfigs", () => {
  const configs = ref([]);
  ...
  const loadConfigs = async ({ force = false, limit = FETCH_LIMIT } = {}) => {
    ...
    error.value = null;
    inflightPromise = api.storage.getStorageConfigs({ limit }).then((response) => {
  ...
  const loadStorageTypes = async ({ force = false } = {}) => {
    ...
    inflightStorageTypesPromise = api.mount.getStorageTypes().then((resp) => {
