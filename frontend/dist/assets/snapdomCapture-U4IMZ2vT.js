const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/snapdom-DlDAnO_e.js","assets/index-BQxzU9F1.js"])))=>i.map(i=>d[i]);
import { c as createLogger, _ as __vitePreload } from "./index-BQxzU9F1.js";
...
async function resolveShareUrlProxyPrefixIfAllowed() {
  try {
    const { useAuthStore } = await __vitePreload(async () => {
      const { useAuthStore: useAuthStore2 } = await import("./index-BQxzU9F1.js").then((n) => n.f7);
      return { useAuthStore: useAuthStore2 };
    }, true ? [] : void 0);
    ...
    const now = Date.now();
    if (cachedPasteUrlProxyTicket && now < cachedPasteUrlProxyTicketExpiresAtMs - 1e4) {
      return `${DEFAULT_PASTE_URL_PROXY_PREFIX}?ticket=${encodeURIComponent(cachedPasteUrlProxyTicket)}&url=`;
    }
    const { post } = await __vitePreload(async () => {
      const { post: post2 } = await import("./index-BQxzU9F1.js").then((n) => n.f5);
      return { post: post2 };
    }, true ? [] : void 0);
    ...
    cachedPasteUrlProxyTicketExpiresAtMs = expiresAtSeconds > 0 ? expiresAtSeconds * 1e3 : now + 5 * 60 * 1e3;
...
function buildSnapdomPngOptions(imageOptions = {}, snapdomOptions = {}) {
  const pixelRatio = typeof imageOptions.pixelRatio === "number" ? imageOptions.pixelRatio : void 0;
  const canvasWidth = typeof imageOptions.canvasWidth === "number" ? imageOptions.canvasWidth : void 0;
  const canvasHeight = typeof imageOptions.canvasHeight === "number" ? imageOptions.canvasHeight : void 0;
  const base = {
    fast: true,
    cache: "soft",
    embedFonts: true,
...
async function elementToPng(element, options = {}) {
  ...
  try {
    ...
    const snapdomModule = await __vitePreload(() => import("./snapdom-DlDAnO_e.js").then((n) => n.s), true ? __vite__mapDeps([0,1]) : void 0);
    ...
    if (!hasExplicitUseProxy && autoUseProxy === true) {
      ...
      if (prefix) {
        ...
        warnings.push({
          ...
          message: "检测到跨域图片资源，但当前未登录或缺少 TEXT_SHARE 权限，或无法获取 proxy ticket，无法使用 /api/paste/url/proxy；导出将使用占位符或跳过跨域资源。"
        });
      }
    }
    ...
    const uniqueScaleCandidates = Array.from(new Set(scaleCandidates)).filter((v) => typeof v === "number" && v > 0).map((v) => Math.min(v, maxSafeScale));
    let blob = null;
    for (const scale of uniqueScaleCandidates) {
      const pngOptions = { ...basePngOptions, scale };
      const candidate = await (typeof snapdom.toBlob === "function" ? snapdom.toBlob(targetElement, pngOptions) : (async () => {
...
async function editorContentToPng(editor, options = {}) {
  if (!editor) {
  ...
  try {
    ...
    if (preferRenderedPreview && livePreviewRoot) {
      ...
      if (!targetElement) {
        targetElement = document.querySelector(".vditor-preview .vditor-reset") || document.querySelector(".vditor-preview") || document.querySelector(".vditor-wysiwyg") || document.querySelector(".vditor-ir") || document.querySelector(".vditor-sv");
      }
    }
